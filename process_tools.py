...
def _silent_official_executable(
    path: str | Path,
    import_name_offset: Callable[[str | Path, str, str], int] = _pe_import_name_offset,
) -> bytes:
    data = bytearray(Path(path).read_bytes())
    offset = import_name_offset(path, "USER32.dll", "MessageBeep")
...
def _console_capture_command(process_id: int, snapshot_path: Path) -> list[str]:
    ...
    return [
        sys.executable,
        str(Path(__file__).with_name("wolf_tools.py").resolve()),
        CONSOLE_CAPTURE_ARG,
        str(process_id),
        str(snapshot_path),
    ]
